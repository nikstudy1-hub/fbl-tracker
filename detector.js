...
function loadThresholds(){
  ...
  return {
    ...
    kickGyro: c.kickGyro || 900,     // °/с — выше «бегового» вращения
    impactAcc: c.impactAcc || 5.0,   // g — выше удара стопы при беге
...
class Detector {
  ...
  push(ax,ay,az,gx,gy,gz){
    ...
    if(this.air){
      ...
      // --- удар / столкновение ---
      if(!this.cap && !inCool && (gMag>T.kickGyro || aMag>T.impactAcc)){ this.cap=true; this.capStart=now; this.pa=aMag; this.pg=gMag; }
      if(this.cap){
        if(aMag>this.pa)this.pa=aMag; if(gMag>this.pg)this.pg=gMag;
        if(now-this.capStart>=T.capMs){
          const type = this.pg>T.kickGyro ? 'KICK' : 'IMPACT';
          this.emit(type,{a:this.pa,g:this.pg}); this.cap=false; this.cool=now+T.coolMs;
        }
      }
    }
